...
The UI is built with SwiftUI as an `LSUIElement` menu-bar app. The app bundle `Info.plist` is generated by `build.sh`; `NetSpeedMonitor/Info.plist` is only a source-tree placeholder. The generated bundle includes `NSAudioCaptureUsageDescription` because the per-app audio mixer uses local CoreAudio process taps.
